...
export function ReportModal({ isOpen, onClose, questions }: ReportModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
            <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col animate-in zoom-in-95 duration-300 relative border border-gray-100">

                {/* Close Button - Stays fixed even when scrolling */}
                <button
                    onClick={onClose}
                    className="absolute top-6 right-6 z-[60] bg-gray-100 hover:bg-gray-200 text-gray-900 p-2.5 rounded-full transition-all hover:rotate-90 shadow-md border border-gray-200 group/close"
                    title="Close and return to dashboard"
                >
                    <X className="w-6 h-6 group-hover/close:scale-110 transition-transform" />
                </button>

                {/* Main Scrollable Content Area */}
                <div className="overflow-y-auto flex-1 rounded-[2.5rem]">
                    {/* Header Decoration */}
                    <div className="absolute top-0 left-0 right-0 h-48 bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 -z-10"></div>
                    ...
                    <div className="relative p-8 md:p-12">
                        <div className="flex justify-between items-start mb-12">
                            <div>
                                ...
                                <p className="text-gray-400 font-medium tracking-wide">Session Summary • {new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</p>
                            </div>
                        </div>

                        {/* Quick Stats */}
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                            <div className="bg-gray-50 p-6 rounded-3xl border border-gray-100 flex items-center gap-5 group/stat hover:bg-white hover:shadow-xl hover:shadow-gray-200/50 transition-all cursor-default">
                                <div className="w-14 h-14 bg-blue-100 rounded-2xl flex items-center justify-center text-blue-600 group-hover/stat:scale-110 transition-transform shadow-sm">
                                    <Users className="w-7 h-7" />
                                </div>
                                <div>
                                    <div className="text-sm font-bold text-gray-500 uppercase tracking-widest">Total Proposals</div>
                                    <div className="text-3xl font-black text-gray-900">{totalQuestions}</div>
                                </div>
                            </div>

                            <div className="bg-gray-50 p-6 rounded-3xl border border-gray-100 flex items-center gap-5 group/stat hover:bg-white hover:shadow-xl hover:shadow-gray-200/50 transition-all cursor-default">
                                <div className="w-14 h-14 bg-orange-100 rounded-2xl flex items-center justify-center text-orange-600 group-hover/stat:scale-110 transition-transform shadow-sm">
                                    <TrendingUp className="w-7 h-7" />
                                </div>
                                <div>
                                    <div className="text-sm font-bold text-gray-500 uppercase tracking-widest">Public Engagement</div>
                            ...
                            <div className="bg-gray-50 p-6 rounded-3xl border border-gray-100 flex items-center gap-5 group/stat hover:bg-white hover:shadow-xl hover:shadow-gray-200/50 transition-all cursor-default">
                                <div className="w-14 h-14 bg-emerald-100 rounded-2xl flex items-center justify-center text-emerald-600 group-hover/stat:scale-110 transition-transform shadow-sm">
                                    <BarChart3 className="w-7 h-7" />
                                </div>
                                <div>
                                    <div className="text-sm font-bold text-gray-500 uppercase tracking-widest">Resolution Rate</div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                            ...
                            <div className="bg-[#020617] p-8 rounded-[2rem] text-white overflow-hidden relative shadow-2xl">
                                {/* Glow */}
                        ...
                        <div className="mt-12 pt-8 border-t border-gray-100 flex flex-col md:flex-row justify-between items-center gap-6">
                            ...
                            <div className="flex gap-4">
                                <button
                                    onClick={onClose}
                                    className="bg-white hover:bg-gray-50 text-gray-700 px-6 py-3 rounded-xl font-bold transition border border-gray-200"
                                >
                                    Back to Dashboard
                                </button>
                                <button
                                    onClick={() => window.print()}
                                    className="bg-gray-900 hover:bg-black text-white px-6 py-3 rounded-xl font-bold transition flex items-center gap-2 shadow-lg shadow-gray-200"
                                >
